...
# ==========================================
# 1. DYNAMIC INPUTS & CARRY-OVER BATON PASS
# ==========================================
YEAR = 2026
MONTH = 5
PTO_REQUESTS = {}
...
if os.path.exists("ui_inputs.json"):
    with open("ui_inputs.json", "r") as f:
        ui_data = json.load(f)
        YEAR = ui_data.get("TARGET_YEAR", 2026)
        MONTH = ui_data.get("TARGET_MONTH", 5)
        PTO_REQUESTS = ui_data.get("PTO_REQUESTS", {})
        REQUESTED_DAYS_OFF = ui_data.get("REQUESTED_DAYS_OFF", {})
        HOLIDAYS = ui_data.get("HOLIDAYS", {})
...
def load_carry_over_data(target_year, target_month):
    ...
    if rows:
        ...
        return {
            ...
            "Emilio Gonzalez": {"weekly_hours": 35, "days_worked_this_week": 4, "has_had_short_day": True, "last_shift_end_time": datetime(2026, 4, 30, 18, 0), "consecutive_days": 4, "last_worked_date": datetime(2026, 4, 30), "night_shifts_this_month": 0},
...
# ==========================================
# 5. EXPORT AND MATRIX FORMATTING
# ==========================================
df_raw = pd.DataFrame(master_schedule_data)
if df_raw.empty:
    print("No shifts scheduled. Exiting.")
    exit()
